...
defmodule StelganoWeb.ChatLiveTest do
  ...
  describe "GET /chat (no handoff)" do
    ...
    test "shows Generate New control for opening the generator drawer", %{conn: conn} do
      {:ok, _view, html} = live(conn, ~p"/chat")
      assert html =~ "Generate New"
      assert html =~ ~s(phx-click="open_generator")
    end
    ...
    test "entry screen does not reveal any conversation history", %{conn: conn} do
      ...
      # Refute "<thing> history" leakage rather than the bare word, since the
      # generator drawer reassures users their generated number is "not saved
      # to history" — privacy-positive copy that should remain visible.
      refute visible =~ ~r/(chat|conversation|message)\s+history/i
    end
  ...
  # ---------------------------------------------------------------------------
  # Entry screen — phone handoff via sessionStorage on post-payment return.
  # PaymentInitiator stashes the phone before checkout; AnonChat reads and
  # clears it on /chat mount. Also exercised by the in-chat generator drawer
  # (which sets phone_locked=true via apply_generated_number, not prefill).
  # ---------------------------------------------------------------------------

  describe "prefill_phone event (post-payment return handoff)" do
  ...
  describe "deriving state" do
    test "entry_submit transitions to deriving", %{conn: conn} do
      ...
      view
      |> form("#entry-form", %{"s_num" => "+12025551234", "s_key" => "1234"})
      |> render_submit()
