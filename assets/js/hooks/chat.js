...
export const AnonChat = {
  mounted() {
    // Post-payment handoff via sessionStorage. PaymentInitiator stashes the
    // user's phone before redirecting to checkout; when the user returns to
    // /chat from /payment/callback we re-prefill the entry form so they
...
// ---------------------------------------------------------------------------
// PhoneInput hook — international formatting and country inference
// ---------------------------------------------------------------------------

export const PhoneInput = {
  mounted() {
    const isManual = this.el.dataset.manual === "true";
    ...
    this.el.addEventListener("input", async () => {
      ...
      this.pushEvent("phone_country_inferred", { country: country || null });
...
// ---------------------------------------------------------------------------
// PaymentInitiator hook — used by both the :new_channel "Choose paid" button
// and the :chat "Extend" button inside ChatLive.
// ---------------------------------------------------------------------------
