...
defmodule StelganoWeb.Plugs.SecurityHeaders do
  ...
  The `/chat` and `/steg-number` routes are set to `noindex, nofollow`.
  A person searching for sTELgano should find the homepage, not the chat
  entry screen. Indexed chat URLs would also leak partial browser history
  to search engine crawlers in some browser extensions.
  ...
  `/chat` and `/steg-number` set `no-store` to prevent the browser from
  caching the page content in the disk or memory cache. This is an additional
  Passcode Test compliance measure — a cached page could be found via
  Ctrl+H or browser cache inspection.
  """

  import Plug.Conn

  @sensitive_paths ["/chat", "/steg-number"]
