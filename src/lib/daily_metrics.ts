...
type DailyColumn = "free_new" | "paid_new" | "free_expired" | "paid_expired";
...
export async function listRecent(db: D1Database, days = 30): Promise<DailyRow[]> {
  ...
  const result = await db
    .prepare(
      "SELECT day, free_new, paid_new, free_expired, paid_expired " +
...
async function bump(db: D1Database, column: DailyColumn, count: number): Promise<void> {
  ...
  // `count` on UPDATE.
  const cols: DailyColumn[] = ["free_new", "paid_new", "free_expired", "paid_expired"];
